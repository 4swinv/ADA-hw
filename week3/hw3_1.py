...
def iteratively_reweighted_shrnkage(sample_size,k,y,l):
    # initialize theta using the solution of regularized ridge regression
    theta = theta_prev = np.linalg.solve(k.T.dot(k) + 1e-4 * np.identity(k.shape[1]), k.T.dot(y))
    #print(theta)
    iter = 0
    error = np.Inf
    while error > 1e-3 and iter < 1000 :
        Phi = np.diag(np.absolute(np.ravel(theta)))
        Phi_gi = np.linalg.pinv(Phi)
        theta = np.linalg.solve(k.T.dot(k) + l * Phi_gi, k.T.dot(y[:, None]))
        error = np.linalg.norm(theta-theta_prev)
        theta_prev = theta
        iter+=1
        #print(error)
...
l = 0.3
theta1 = iteratively_reweighted_shrnkage(sample_size,k,y,0.1)
